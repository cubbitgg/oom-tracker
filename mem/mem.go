...
func NumaStatForProc(proc *os.Process) (string, error) {
	path, err := os.ReadFile(fmt.Sprintf("/proc/%d/cgroup", proc.Pid))
	if err != nil {
		return "", err
	}
	...
	path = bytes.TrimSpace(path[pbegin:])
	spath := fmt.Sprintf("/sys/fs/cgroup/%s/memory.numa_stat", string(path))
	return readStringFromFile(spath)
}

func readStringFromFile(fpath string) (string, error) {
	content, err := os.ReadFile(fpath)
	if err != nil {
		return "", err
	}
